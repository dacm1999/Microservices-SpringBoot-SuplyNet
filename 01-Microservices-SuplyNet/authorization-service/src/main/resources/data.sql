...
-- Insertar el cliente de Swagger
insert into client (client_id, client_secret, duration_in_minutes, required_proof_key) values ('swagger-ui-client', '$2a$12$oU698BnrvIb4XV6HYFDLKeQW9wbjo8H.1SOdXYrZagU7XFvf8Iy1i', 10, false);
insert into client_authorization_grant_types (client_id, authorization_grant_types) values ((select id from client where client_id = 'swagger-ui-client'), 'authorization_code');
insert into client_authorization_grant_types (client_id, authorization_grant_types) values ((select id from client where client_id = 'swagger-ui-client'), 'refresh_token');
insert into client_client_authentication_methods (client_id, client_authentication_methods) values ((select id from client where client_id = 'swagger-ui-client'), 'client_secret_basic');
insert into client_redirect_uris (client_id, redirect_uris) values ((select id from client where client_id = 'swagger-ui-client'), 'http://localhost:8080/swagger-ui/oauth2-redirect.html');
insert into client_scopes (client_id, scopes) values ((select id from client where client_id = 'swagger-ui-client'), 'openid');
insert into client_scopes (client_id, scopes) values ((select id from client where client_id = 'swagger-ui-client'), 'profile');
